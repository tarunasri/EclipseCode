<%-- $Id: CpuReportWidget.jsp,v 1.2 2010/10/29 13:47:09 swaminathap Exp $ --%>
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:1px;_margin-top:0px;">
    <tr class="dgeneralTBHeader" style="font-weight:bold;">
                <td style="padding-left:7px;"><fmt:message key = 'webclient.home.dashboards.report.table.name'></fmt:message></td>
                <td style="padding-left:7px;align:left;" ><fmt:message key = 'webclient.home.dashboards.report.table.avg'><fmt:param><c:out value="${units}"/></fmt:param></fmt:message></td>
                </tr>
...
<c:when test="${!empty reportData}">
	...
	<tr  class="generalEvenRow" >
		...
		<tr class="generalOddRow" >
			...
			<td style="padding-left:7px;width:70%" align="left" class="text">
...
<div class="graphGreenBg2">
								...
								<table style="line-height:1px;font-size:3px;" width="<c:out value='${prop.Value}'/>" cellspacing="0" cellpadding="0" border="0">
									<tr>
										...
										<td  style="background:rgb(211,214,215);"><img height="8" width="1" src="/webclient/common/images/spacer.gif"/> </td>

										</c:when>
										<c:otherwise>
										<c:choose>
										<c:when test="${prop.Value > 80}">
										<td style="background: rgb(200,68,63);"><img height="8" width="1" src="/webclient/common/images/spacer.gif"/> </td>
										</c:when>
										<c:otherwise>

										<td style="background: rgb(141,199,63);"><img height="8" width="1" src="/webclient/common/images/spacer.gif"/> </td>
										</c:otherwise>
										</c:choose>
										</c:otherwise>
										</c:choose>
						...
						<td nowrap="" align="right" style="padding-left: 5px;padding-right:5px;"><c:out value='${prop.Value}'/></td>
						</c:when>
						<c:otherwise>

						<td nowrap="" align="left" style="padding-left: 5px;padding-right:5px;"><c:out value='${prop.Value}'/></td>
