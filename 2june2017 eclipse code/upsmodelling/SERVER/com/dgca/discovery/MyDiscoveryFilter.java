...
import com.dgca.discovery.DbConnection;
...
public class MyDiscoveryFilter implements FoundFilter {
	...
	String hostNmae=null;

	public MyDiscoveryFilter() {
...
if((mo.getType().equals("APC"))){
	...
	try {
		...
		if (mo == null)
			return null;
		if (mo.getType().equals("Interface")) {
			//String hostName = dbconn.gethostName(mo.getName(), conn);
			String name = mo.getName();
			String hostName="";
			if(name.startsWith("IF-")){
				name = name.substring(3, name.length());
				 hostName = dbconn.gethostName(name, conn);
				 hostName = "IF-"+name;
			}
			else{
				hostName = dbconn.gethostName(name, conn);
			}
			System.err.println("HostName from Interface..." + hostName);

			//System.out.println("In discovery Filter NON SNMP NODE BLOCK");
			mo.setName(hostName);
			mo.setDisplayName(hostName);
			return mo;
	}	
		...
		if (!(mo instanceof SnmpNode)) {
			...
			if (mo.getType().equals("Network")) {
				System.err.println("HostName from DGCAMAPPING...Network");

				return mo;
			}	
			if (mo.getType().equals("Node")) {
				String name = mo.getName();
				String hostName="";
				if(name.startsWith("IF-")){
					name = name.substring(3, name.length());
				...
				System.err.println("HostName from SnmpNode..." + hostName);
		...
		System.out.println("MO ADDITON FINAL"+mo.getName());
